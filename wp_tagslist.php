...
function load_taglist_template () {
	if(@file_exists(TEMPLATEPATH.'/taglist.php')) {
		...
		require_once(WP_PLUGIN_DIR.'/wp-taglist/taglist.tpl');
	}
	exit;
}
	...
	if($carakter[0] == $url_str){
		...
		foreach($char_sort_tags[$carakter[0]] as $key => $tag){
				if($permalink==true){
 				...
 				$string .= '<li><a href="'.get_option('home').$link.'">'.$char_sort_tags[$carakter[0]][$key]['name'].'('.$char_sort_tags[$carakter[0]][$key]['count'].') </a></li>';
		}
...
$options = array (
   "0" => array( 
			...
			"content" => array(/*
							array( 
								"type" => "info",
								"style" => "width:90%;border:1px solid #B3DAEF;background:#FFFFE0;",
								"ID" => "plugindescr",
								"class" => "on",
								"value" => '<script>
      														...
      														$("p#info").toggle(
																function(){
																	if(!$("#plugindescr").attr("class")){
																		...
																		$("#plugindescr").attr("class","width:90%;border:1px solid #B3DAEF;background:#FFFFE0;");
																	}else{
																		$("#plugindescr").attr("class","on");
																		$("#plugindescr").attr("style","width:90%;border:1px solid #B3DAEF;");
																	}
																}
															);
    												});
...
Was das wp-taglist Pligin alles kann. <a id="tog" style="cursor:pointer;">'.get_the_translation('Lies es hier![@_L]read it here!').'</a>
<p id="info">Hi ich bin René Reimann - ich möcht mich kurz vortellen und für dein Interesse an diesem Plugin bedanken. Ich bin leidenschaftlicher Mediengestalter für Digital- & Printmedien. Wenn ich nicht gerade tolle Ideen von Inhouse SEO Hossa umsetze dann entwickle ich 1A Fontend- und Backendlösungen oder setze mein Börsenwissen bei Spekulationen an der Börse ein.</p> ',	
								),*/
							array( 
								"type" => "input",
			...
			foreach ($options[$k]['content'] as $key => $value) { 
				...
				if($options[$k]['content'][$key]['type'] == 'textarea'){
					...
					foreach($options[$k]['content'][$key]['value'] as $value => $decs){
						if(get_settings($prefix.'_'.$options[$k]['content'][$key]['id']) == $value){
				...
				echo '</td></tr>';
					...
					if($options[$k]['content'][$key]['type'] == 'textarea'){
						...
						echo '<tr><td style="background-color:#EEEEEE;" colspan="5"><b>'.$options[$k]['content'][$key]['value'].'</b></td></tr>';
